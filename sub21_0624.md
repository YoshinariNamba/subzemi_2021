...
2. 一つの行が一つの観測を表す  
3. 一つのセルが一つの値を表す  
`math_score`や`eng_score`が「教科」と「点数」の2つの意味を含んでしまっていて，`df_score`はTidy
...
```
    ...
    ## 2 生徒C small      eng_score     70
    ## 3 生徒A large      math_score    NA
    ## 4 生徒A large      eng_score     75
    ## 5 生徒B small      math_score    75
    ## 6 生徒B small      eng_score     80
    ## 7 生徒A large      math_score    70
    ## 8 生徒A large      eng_score     75
...
文字ベクトルを連結したいときは`stringr::str_c()`を使います．最初の引数に文字列ベクトルを

``` r
str_c(vec_chr, collapse = '・')
```
