...
グループワークです．2019年三田論製薬班の再現です．（お兄さんにデータを提供していただきました．ありがとうございます！）

## データ

使用するデータは`inventor.txt`と`pharma_patent.xlsx`です．いずれも特許に関するデータですが，前者は開発者個人の氏名，後者は企業名が観測されています．
.txtファイルは次のように読み込みます．

``` r
# データの取り込み1: テキストデータ
df_inventor <- read.table('inventor.txt', header = T, sep = ',')
...
# パッケージの呼び出し
library(openxlsx)
df_pharma <- read.xlsx('pharma_patent.xlsx')
